...
void construct(vector<int> inp, int cur_node, int arr_l, int arr_r, int segtree[]) {
    if (arr_l == arr_r) {
        ...
        segtree[cur_node] = min(segtree[cur_node*2], segtree[cur_node*2+1]);
    }
}

//O(logn)
int min_val(int cur_node, int arr_l, int arr_r, int query_l, int query_r, int segtree[]) {
    if (query_l > query_r) 
        //if invalid query
        return 2147483647;
    ...
    return min(min_val(cur_node*2, arr_l, arr_m, query_l, min(query_r, arr_m), segtree), min_val(cur_node*2+1, arr_m+1, arr_r, max(query_l, arr_m+1), query_r, segtree));
}
...
void update(int cur_node, int arr_l, int arr_r, int upd_pos, int new_val, int segtree[]) {
    if (arr_l == arr_r) {
        ...
        //calculate min value based on new value for 1 of the children
        segtree[cur_node] = min(segtree[cur_node*2], segtree[cur_node*2+1]);
    }
}
    ...
    for (int i=0; i<q; i++) {
        ...
        if (a==1) {
            ...
            */
            ...
            cout << min_val(1, 0, x.size()-1, b, c, seg) << endl;
        }
    }
